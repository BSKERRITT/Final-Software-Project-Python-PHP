...
cam.framerate = 32
rawCapture = PiRGBArray(cam, size=(300, 200))
...
for frame in cam.capture_continuous(rawCapture, format="bgr", use_video_port=True):    
    feed = frame.array
    #feed = imutils.resize(feed, width=300)
    queryImg = cv2.cvtColor(feed, cv2.COLOR_BGR2GRAY)
    ...
    if(len(goodMatch) > MIN_MATCH_COUNT):
        ...
        cv2.polylines(feed,[np.int32(queryBorder)], True, (0,255,0), 5)
    else:
